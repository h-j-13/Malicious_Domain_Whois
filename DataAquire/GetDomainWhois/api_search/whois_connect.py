...
flag_proxy = False  # 使用代理标志
_proxy_ip = None
if flag_proxy:
    _proxy_ip = ProxyIP()  # 代理IP获取对象
...
class GetWhoisInfo:
    ...
    @staticmethod
    def _is_error(data):
        return True if (data in error_list or data != None) else False
...
def test():
    domain = 'bluessocietyofaustin.org'  # raw_input("domain :")
    whois_server = 'whois.pir.org'  # raw_input("whois_server :")
    import time
